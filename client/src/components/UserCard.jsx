  ...
  return (
    <div onClick={() => navigate(`/profile/${user._id}`)} key={user._id} className='p-4 pt-6 flex flex-col justify-between w-72 shadow border border-gray-200 rounded-md cursor-pointer'>
        <div className='text-center'>
            <img src={user.profile_picture} alt="Profile Picture" className='rounded-full w-16 h-16 shadow-md mx-auto object-cover' loading='lazy' decoding="async" onLoad={() => setLoaded(true)} style={{filter: loaded ? "none" : "blur(20px)", transition: "filter 0.3s ease-out"}} />
        ...
        <div className='flex mt-4 gap-2'>
            {/* Follow Button */}
            <button onClick={(e) => {e.stopPropagation(); handleFollow();}} disabled={currentUser?.following.includes(user._id)} className='w-full py-2 rounded-md flex justify-center items-center gap-2 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 active:scale-95 transition text-white cursor-pointer'>
                <UserPlus className='w-4 h-4' /> {currentUser?.following.includes(user._id) ? "Following" : "Follow"}
            </button>

            {/* Connection Request & Message Button */}
            <button onClick={(e) => {e.stopPropagation(); handleConnectionRequest();}} className='flex items-center justify-center w-16 border text-slate-500 group rounded-md cursor-pointer active:scale-95 transition'>
                {
